...
async function main() {
  ...
  console.log("\n🚀 Deploying CredHub contracts to Polygon mainnet...");
  ...
  const filename = `./deployed-config-${network.name}.json`;
  fs.writeFileSync(filename, JSON.stringify(deploymentInfo, null, 2));
  console.log(`📄 Deployment config saved to ${filename}`);
  
  // Verify contracts on Polygonscan (if API key is provided)
  if (process.env.POLYGONSCAN_API_KEY && (network.name === "polygon" || network.name === "matic")) {
  ...
  console.log("\n🔗 View on Polygonscan:");
  console.log(`IssuerRegistry: https://polygonscan.com/address/${issuerRegistryAddress}`);
  console.log(`CredentialRegistry: https://polygonscan.com/address/${credentialRegistryAddress}`);
  ...
  console.log("4. Monitor contract usage and gas costs");
}
